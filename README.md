...
第一阶段（数据底座与最小查询闭环）已完成，当前进入**第二阶段小步迭代**。

当前短期重点：

1. 持续提升“单物品 TID 查询”的可用性与解释性
2. 围绕 `query_item` / `build_indexes` / `domain_hints` 做小步补丁迭代
3. 继续扩展任务、礼包、强化等关键业务路径的候选关联覆盖
4. 严格维持“候选 / 待确认”边界，避免把不确定语义写死

当前阶段**不做在线编辑，不写回原始表，不进入正式前端开发**。
